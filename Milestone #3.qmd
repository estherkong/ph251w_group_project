...
# clean up county names for joining with the population dataset
morbidity_combined$county <- gsub(" County","",morbidity_combined$county)
...
stratified_morbidity_infections
...
#grouping - Esther feel free to add what you mentioned 
...
```{r}
# Join the morbidity & population datasets

#group ca_pop_clean first before combining with stratified morbidity 

ca_pop_clean_grp <- ca_pop_clean %>%
  group_by(county, race_ethnicity) %>% 
  summarize(pop = sum(pop))

#shows county x race_ethnicity with breakdowns for new infections and severe cases out of total population (all the race_ethnicity category of each county)

all_three_joined <- left_join(stratified_morbidity_infections, ca_pop_clean_grp, 
...
rate_infections <- all_three_joined %>%
  group_by(county, race_ethnicity) %>% 
  mutate(`infection rate` = (`new infections` / `pop`) * 100 )

#county strata of interest 

#add pop of all counties into one row and final rate metric

rate_infections_stratified <- rate_infections %>%
  group_by(county) %>% 
  summarize(infections = sum(`new infections`),
            pop = sum(pop)) %>%
  mutate(`infection rate by county` = (`infections` / `pop`) * 100) 
rate_infections_stratified
