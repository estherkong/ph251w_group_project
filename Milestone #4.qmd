...
## Table of Infections per Race/County
...
## Plot of infection distributions by age

```{r}
# re-stratify datasets by age categories
age_ca_pop <- ca_pop_clean %>%
  group_by(age_category) %>% 
  summarize(pop = sum(pop)) %>% 
  ungroup()

age_morbidity_infections <- morbidity_combined %>% 
  group_by(age_category) %>% 
...
age_joined <- left_join(age_morbidity_infections,
                              age_ca_pop, 
                              by = c("age_category"))

# calculate infection rates per 100 ppl for age groups and pivot for plotting
age_rates <- age_joined %>% 
  mutate(new_inf_rate = round((new_infections/pop)*100, digits=3),
         new_severe_rate = round((new_severe/pop)*100,digits=3)) %>% 
  pivot_longer(cols = c(new_inf_rate, new_severe_rate),
               names_to = "variable", values_to = "rate") %>% 
  mutate(variable = recode(variable,
                           new_inf_rate = "New infections",
                           new_severe_rate = "New severe infections"))

# create plot
age_plot <- ggplot(data = age_rates,
                   aes(x = age_category, y = rate, fill=variable)) +
  geom_col(position = "dodge") +
  labs(fill = "Infection type",
       title = "New and Severe Infections in California by Age Group",
       y= "New infections per 100 people",
       x = "Age group",
       caption = "New infections rate and new severe infections rate is highest among those aged 65+.")
age_plot
```
